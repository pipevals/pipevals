 ...
 *
 * Each upstream result is keyed by both node ID and slug (when present)
 * so that dot-paths authored with either reference style resolve correctly.
 * Auto-wire generates slug-based paths (e.g. `steps.llm.text`), while
...
export function resolveInputs(
  ...
  for (const edge of incomingEdges) {
    ...
    if (sourceOutput) {
      ...
      if (sourceNode?.slug) {
        steps[sourceNode.slug] = sourceOutput;
      }
    }
  }
