...
The system SHALL render an xyflow canvas where users can view and interact with pipeline nodes and edges. The canvas MUST support pan, zoom, and minimap navigation. The canvas MUST load its state from the database and persist changes on save.

#### Scenario: Load pipeline into canvas
- **WHEN** a user navigates to a pipeline's editor page
- **THEN** the canvas renders all nodes at their stored positions with edges connecting them
...
The system SHALL allow users to draw edges between nodes by dragging from an output handle to an input handle. Condition nodes MUST display labeled output handles (e.g., "true"/"false" or custom labels). The system MUST prevent connections that would create a cycle.
