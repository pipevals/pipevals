...
The system SHALL expose `POST /api/pipelines` to create a new pipeline. The request body MUST include name and optional description. The response MUST return the created pipeline with its id. The pipeline MUST be scoped to the authenticated user's active organization.
...
- **THEN** the system returns 201 with the created pipeline including its generated id
...
The system SHALL expose `GET /api/pipelines` to list all pipelines in the authenticated user's active organization. The response MUST include each pipeline's id, name, description, and timestamps.

#### Scenario: List pipelines
- **WHEN** an authenticated user sends `GET /api/pipelines` and their organization has 3 pipelines
- **THEN** the system returns 200 with an array of 3 pipeline objects

### Requirement: Get pipeline with graph
The system SHALL expose `GET /api/pipelines/:id` to retrieve a pipeline with its full graph (nodes and edges). The response MUST include the pipeline metadata plus `nodes` and `edges` arrays. The response shape MUST be directly consumable by xyflow (nodes with id, type, position, data; edges with id, source, target, sourceHandle, targetHandle).

#### Scenario: Get pipeline with nodes and edges
- **WHEN** an authenticated user requests a pipeline that has 4 nodes and 3 edges
- **THEN** the system returns 200 with pipeline metadata, a nodes array of 4 items, and an edges array of 3 items
