...
The system SHALL store pipeline nodes with an id, pipeline_id (foreign key), type enum, label, config (jsonb), position_x (real), and position_y (real). The type enum MUST include: `api_request`, `ai_sdk`, `sandbox`, `condition`, `transform`, `metric_capture`. Deleting a pipeline MUST cascade-delete all its nodes.

#### Scenario: Add a node to a pipeline
- **WHEN** a node of type `ai_sdk` is added with config `{ "model": "gpt-4o", "provider": "openai" }` and position (200, 100)
- **THEN** the system persists the node linked to its pipeline with the given type, config, and position
