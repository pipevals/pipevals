...
The system SHALL expose a `GET /api/pipelines/:id/runs/metrics` endpoint that returns metric values and step durations for all completed runs of a pipeline. The endpoint MUST require authentication and verify the user has access to the pipeline's organization. The response MUST be a JSON object with a `runs` array.
