...
The system SHALL define an `OutputPort` type with a `key` field (the output object key that downstream steps reference via dot-paths). A step type with an empty outputs array MUST be skipped as an auto-wire source. The output port's `key` is used to construct the dot-path: `steps.<sourceLabel>.<key>`.

#### Scenario: Step type with declared output

- **WHEN** an `ai_sdk` step with label "llm" has output port `{ key: "text" }` and is connected to a downstream node
