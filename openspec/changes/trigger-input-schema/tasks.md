...
- [ ] 1.1 Add `trigger_schema` nullable JSONB column to the `pipelines` table in `lib/db/pipeline-schema.ts`
...
- [ ] 2.1 Update `GET /api/pipelines/:id` to include `triggerSchema` in the response (default `[]` for null)
...
- [ ] 6.1 Unit test: `GET /api/pipelines/:id` returns `triggerSchema` field (defaulting to `[]`)
- [ ] 6.2 Unit test: `PUT /api/pipelines/:id` persists `triggerSchema`; omitting the field preserves existing schema
- [ ] 6.3 Unit test: graph snapshot excludes trigger node and trigger edges
- [ ] 6.4 Unit test: store `addTriggerField` / `removeTriggerField` update schema and trigger node handles
- [ ] 6.5 Unit test: edge from trigger handle writes correct `trigger.{fieldName}` into target step config
- [ ] 6.6 Unit test: trigger node cannot be deleted (delete action is a no-op)
