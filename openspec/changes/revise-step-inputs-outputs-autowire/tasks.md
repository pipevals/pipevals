...
- [ ] 1.1 Update `MetricCaptureConfig` type in `lib/pipeline/types.ts` to `{ type: "metric_capture"; metrics: Record<string, string> }`
- [ ] 1.2 Update `metricCaptureConfigSchema` in `lib/pipeline/config-schemas.ts` to use `metrics: z.record(z.string(), z.string())`
- [ ] 1.3 Update `defaultConfigs.metric_capture` in `lib/pipeline/types.ts` to `{ type: "metric_capture", metrics: {} }`
...
- [ ] 2.1 Change `PRIMARY_INPUT_FIELD.api_request` from `"url"` to `"__body__"` in `lib/pipeline/auto-wire.ts`
- [ ] 2.2 Add `__body__` special-case handler in `autoWireInputs` that additively merges `{"": dotPath}` into `bodyTemplate` (mirrors `__mapping__` logic)
- [ ] 2.3 Change `PRIMARY_INPUT_FIELD.sandbox` from `null` to `"code"` in `lib/pipeline/auto-wire.ts`
