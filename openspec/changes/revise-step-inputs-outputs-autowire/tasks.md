...
- [x] 2.5 Change condition auto-wire in `autoWireInputs` so the value inserted into `expression` is `${dotPath} != null` instead of the bare dotPath
- [x] 2.6 Change `PRIMARY_INPUT_FIELD.metric_capture` from `"extractPath"` to `"__metrics__"` in `lib/pipeline/auto-wire.ts`
- [x] 2.7 Add `__metrics__` special-case handler in `autoWireInputs` that additively merges `{"": dotPath}` into `metrics` (mirrors `__mapping__` logic)
